...
public class WebScreenshotUtils {
    ...
    public static String saveWebPageScreenshot(String webUrl){
        ...
        try {
            ...
            log.info("原始图片保存路径:{}",imageSavePath);
            //压缩图片
            final String COMPRESSED_SUFFIX = "_compressed.jpg";
            String compressedImageSavePath = rootPath + File.separator + RandomUtil.randomNumbers(5) + COMPRESSED_SUFFIX;
            compressImage(imageSavePath,compressedImageSavePath);
            log.info("压缩图片保存路径:{}",compressedImageSavePath);
            //删除原始路径
    ...
    public static void compressImage(String originImagePath,String compressedImagePath){
        //压缩图片质量（0.1 = 10%）
        final float CONPRESSION_QUALITY = 0.3f;
        try{
            ImgUtil.compress(
