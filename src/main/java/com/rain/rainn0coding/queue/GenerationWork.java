...
public class GenerationWork {
    ...
    public GenerationWork(AiCodeGeneratorFacade facade,PythonAiClient python,AppService apps,UserService users,ChatHistoryService history) {
        this.facade=facade;this.python=python;this.apps=apps;this.users=users;this.history=history;
    }
    ...
    public Flux<String> stream(GenerationTaskStore.Task task) {
        ...
        try {
            return facade.generateAndSaveCodeStream(task.prompt(),CodeGenTypeEnum.getEnumByValue(task.codeGenType()),
                    task.appId(),task.userId(),user.getUserRole(),task.taskId(),task.taskId(),true);
