...
<template>
  <section class="agent-progress" aria-labelledby="agent-progress-title">
    <div class="agent-progress__heading">
      ...
      <span class="agent-progress__run-status" role="status" aria-live="polite" aria-atomic="true">{{ status === 'queued' ? '排队中…' : status === 'paused' ? '已暂停' : status === 'pausing' ? '正在暂停…' : status === 'connecting' ? '正在连接…' : status === 'cancelled' ? '已取消' : status === 'failed' ? '需要处理' : status === 'success' ? '已完成' : status === 'running' ? '生成中…' : '等待开始' }}</span>
    </div>
